...
(function(){
  ...
  (function(){
    var esc=location.pathname.indexOf('/es')===0;
    var f=new Intl.NumberFormat(esc?'es-ES':'en-US');
    ...
    function r(){
      ...
      var money=function(n){return esc?(f.format(n)+'&euro;'):('&euro;'+f.format(n));};
      el('capOut').innerHTML=money(v);
      el('moOut').textContent=f.format(Math.round(mo));
      el('anOut').innerHTML=money(Math.round(an));
      el('decOut').innerHTML=money(Math.round(an*10));
    }
